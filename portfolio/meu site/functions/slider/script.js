...
$(document).ready(function(){
	...
	//set defaults
	$("#plan-holder").text('Até 100 Lançamentos/Mês');
	$(".info-price").html('R$ 0,00 / mês');
	$(".bg-blue").html('Iniciar Gratuito');
	$(".bg-white").hide();
    ...
    animationCallback: function(x, y) {
      ...
      //set personal
      if(slider_value > 0 && slider_value < 14){
		$(".bg-blue").html('Iniciar Gratuito');
		$(".bg-white").hide();
		
		$("#plan-holder").text('Até 100 Lançamentos/Mês');
		$(".info-price").html('R$ 0,00 / mês');
		$("#range-holder-valor").html('R$ 0,00 / mês'); 
		$("#green-highlight").hide(); 
		$(".box-img").hide();
		$(".box-img-1").show();

		$(".status").css("background-color", "#8b8b8b");
      ...
      if(slider_value > 5 && slider_value < 37){
	      ...
	      if(slider_value > 10){
			$(".bg-blue").html('Assine agora');
			$(".bg-white").show();
			$(".info-price").html('R$ 87 / mês'); 
			$("#range-holder-valor").html('R$ 87,00 / mês'); 
			$("#plan-holder").text('Até 30 Automações/Mês');

			$(".status").css("background-color", "#8b8b8b");
			$(".status-1").css("background-color", "#5FC29E");
	      ...
	      if(slider_value > 25){
			$(".info-price").html('R$ 137 / mês'); 
			$("#range-holder-valor").html('R$ 137,00 / mês'); 

			$("#plan-holder").text('Até 100 Automações/Mês');
			...
			$(".status").css("background-color", "#8b8b8b");
			  $(".status-1").css("background-color", "#5FC29E");
			  $(".status-2").css("background-color", "#5FC29E");
	      }	
      ...
      if(slider_value > 38 && slider_value < 83){
	      ...
	      $("#green-highlight").css("width", "250px");
	      ...
	      if(slider_value > 40){
			$("#plan-holder").text('Até 250 Automações/Mês');
			$("#range-holder-valor").html('R$ 199,00 / mês'); 
			$(".info-price").html('R$ 199 / mês'); 
			  
		
			$("#orange-highlight").css("width", ""+(slider_value/5)+"%");
			$(".box-img").hide();
			$(".box-img-4").show();
			...
			$(".status-1").css("background-color", "#5FC29E");
			$(".status-2").css("background-color", "#5FC29E");
			$(".status-3").css("background-color", "#5FC29E");
	      }
	      
	      
	      if(slider_value > 50){
			$("#plan-holder").text('Até 400 Automações/Mês');
			$("#range-holder-valor").html('R$ 299,00 / mês'); 
	      	$(".info-price").html('R$ 299 / mês'); 
			$(".box-img").hide();
			$(".box-img-5").show();
			...
			$(".status-2").css("background-color", "#5FC29E");
			$(".status-3").css("background-color", "#5FC29E");
			$(".status-4").css("background-color", "#5FC29E");
	      }
	      
	      
	      if(slider_value > 60){
			$("#plan-holder").text('Até 900 Automações/Mês');
			$("#range-holder-valor").html('R$ 399,00 / mês'); 
	      	$(".info-price").html('R$ 399 / mês'); 
			$(".box-img").hide();
			$(".box-img-6").show();

			$(".status").css("background-color", "#5FC29E");
			$(".status-6").css("background-color", "#8b8b8b");
			$(".assine-agora").show();
      ...
      //set enterprise
      if(slider_value > 83){
		$("#plan-holder").text('+900 Automações/Mês');
		$("#range-holder-valor").html(''); 
	  	$("#orange-highlight").css("width", "215px");

      	      	
      	$(".info-price").html('Fale com Especialista'); 
		$(".box-img").hide();
		$(".assine-agora").hide();
		$(".box-img-7").show();
